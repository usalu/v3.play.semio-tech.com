...
var b = 1024 * 16, C = typeof TextDecoder < "u" ? new TextDecoder() : typeof Buffer < "u" ? { decode(u) {
...
class g {
  constructor(t, e, i) {
    this.start = t, this.end = e, this.original = i, this.intro = "", this.outro = "", this.content = i, this.storeName = false, this.edited = false, this.previous = null, this.next = null;
  }
  appendLeft(t) {
    this.outro += t;
  }
  ...
  edit(t, e, i) {
    return this.content = t, i || (this.intro = "", this.outro = ""), this.storeName = e, this.edited = true, this;
  }
  prependLeft(t) {
  ...
  split(t) {
    ...
    return r.outro = this.outro, this.outro = "", this.end = t, this.edited ? (r.edit("", false), this.content = "") : this.content = i, r.next = this.next, r.next && (r.next.previous = r), r.previous = this, this.next = r, r;
  }
  toString() {
  ...
  trimEnd(t) {
    ...
    if (e.length) return e !== this.content && (this.split(this.start + e.length).edit("", void 0, true), this.edited && this.edit(e, this.storeName, true)), true;
    if (this.edit("", void 0, true), this.intro = this.intro.replace(t, ""), this.intro.length) return true;
  }
  trimStart(t) {
    ...
    if (e.length) {
      if (e !== this.content) {
        ...
        this.edited && i.edit(e, this.storeName, true), this.edit("", void 0, true);
      }
      return true;
    } else if (this.edit("", void 0, true), this.outro = this.outro.replace(t, ""), this.outro.length) return true;
  }
}
function _() {
...
function A(u, t) {
  ...
  for (e.pop(); e[0] === i[0]; ) e.shift(), i.shift();
  if (e.length) {
    let n = e.length;
    for (; n--; ) e[n] = "..";
  }
...
function y(u) {
  const t = u.split(`
`), e = [];
  for (let i = 0, n = 0; i < t.length; i++) e.push(n), n += t[i].length + 1;
  return function(n) {
    let r = 0, s = e.length;
    for (; r < s; ) {
      const l = r + s >> 1;
...
`, s + 1);
  ...
  addUneditedChunk(t, e, i, n, r) {
    let s = e.start, h = true, o = false;
    for (; s < e.end; ) {
      if (i[s] === `
`) n.line += 1, n.column = 0, this.generatedCodeLine += 1, this.raw[this.generatedCodeLine] = this.rawSegments = [], this.generatedCodeColumn = 0, h = true, o = false;
      else {
        if (this.hires || h || r.has(s)) {
          const l = [this.generatedCodeColumn, t, n.line, n.column];
          this.hires === "boundary" ? $.test(i[s]) ? o || (this.rawSegments.push(l), o = true) : (this.rawSegments.push(l), o = false) : this.rawSegments.push(l);
        }
        n.column += 1, this.generatedCodeColumn += 1, h = false;
      }
      s += 1;
    }
    this.pending = null;
...
`);
    if (e.length > 1) {
      for (let i = 0; i < e.length - 1; i++) this.generatedCodeLine++, this.raw[this.generatedCodeLine] = this.rawSegments = [];
...
class v {
  constructor(t, e = {}) {
    ...
    Object.defineProperties(this, { original: { writable: true, value: t }, outro: { writable: true, value: "" }, intro: { writable: true, value: "" }, firstChunk: { writable: true, value: i }, lastChunk: { writable: true, value: i }, lastSearchedChunk: { writable: true, value: i }, byStart: { writable: true, value: {} }, byEnd: { writable: true, value: {} }, filename: { writable: true, value: e.filename }, indentExclusionRanges: { writable: true, value: e.indentExclusionRanges }, sourcemapLocations: { writable: true, value: new p() }, storedNames: { writable: true, value: {} }, indentStr: { writable: true, value: void 0 }, ignoreList: { writable: true, value: e.ignoreList }, offset: { writable: true, value: e.offset || 0 } }), this.byStart[0] = i, this.byEnd[t.length] = i;
  }
  ...
  generateDecodedMap(t) {
    ...
    return this.intro && n.advance(this.intro), this.firstChunk.eachNext((s) => {
      const h = r(s.start);
      s.intro.length && n.advance(s.intro), s.edited ? n.addEdit(e, s.content, h, s.storeName ? i.indexOf(s.original) : -1) : n.addUneditedChunk(e, s, this.original, h, this.sourcemapLocations), s.outro.length && n.advance(s.outro);
    }), this.outro && n.advance(this.outro), { file: t.file ? t.file.split(/[/\\]/).pop() : void 0, sources: [t.source ? A(t.file || "", t.source) : t.file || ""], sourcesContent: t.includeContent ? [this.original] : void 0, names: i, mappings: n.raw, x_google_ignoreList: this.ignoreList ? [e] : void 0 };
  }
  generateMap(t) {
  ...
  indent(t, e) {
    ...
    e.exclude && (typeof e.exclude[0] == "number" ? [e.exclude] : e.exclude).forEach((a) => {
      for (let w = a[0]; w < a[1]; w += 1) n[w] = true;
    });
    let r = e.indentStart !== false;
    const s = (l) => r ? `${t}${l}` : (r = true, l);
    this.intro = this.intro.replace(i, s);
    let h = 0, o = this.firstChunk;
    for (; o; ) {
      const l = o.end;
      if (o.edited) n[h] || (o.content = o.content.replace(i, s), o.content.length && (r = o.content[o.content.length - 1] === `
`));
      else for (h = o.start; h < l; ) {
        if (!n[h]) {
          const a = this.original[h];
          a === `
` ? r = true : a !== "\r" && r && (r = false, h === o.start || (this._splitChunk(o, h), o = o.next), o.prependRight(t));
        }
        h += 1;
      }
      h = o.end, o = o.next;
  ...
  insertLeft(t, e) {
    return f.insertLeft || (console.warn("magicString.insertLeft(...) is deprecated. Use magicString.appendLeft(...) instead"), f.insertLeft = true), this.appendLeft(t, e);
  }
  insertRight(t, e) {
    return f.insertRight || (console.warn("magicString.insertRight(...) is deprecated. Use magicString.prependRight(...) instead"), f.insertRight = true), this.prependRight(t, e);
  }
  move(t, e, i) {
  ...
  update(t, e, i, n) {
    ...
    this._split(t), this._split(e), n === true && (f.storeName || (console.warn("The final argument to magicString.overwrite(...) should be an options object. See https://github.com/rich-harris/magic-string"), f.storeName = true), n = { storeName: true });
    const r = n !== void 0 ? n.storeName : false, s = n !== void 0 ? n.overwrite : false;
    if (r) {
      const l = this.original.slice(t, e);
      Object.defineProperty(this.storedNames, l, { writable: true, value: true, enumerable: true });
    }
    const h = this.byStart[t], o = this.byEnd[e];
    if (h) {
      let l = h;
      for (; l !== o; ) {
        if (l.next !== this.byStart[l.end]) throw new Error("Cannot overwrite across a split point");
        l = l.next, l.edit("", false);
      }
      h.edit(i, r, !s);
    } else {
  ...
  remove(t, e) {
    ...
    for (; i; ) i.intro = "", i.outro = "", i.edit(""), i = e > i.end ? this.byStart[i.end] : null;
    return this;
  }
  reset(t, e) {
    if (t = t + this.offset, e = e + this.offset, this.original.length !== 0) {
    ...
    this._split(t), this._split(e);
    let i = this.byStart[t];
    for (; i; ) i.reset(), i = e > i.end ? this.byStart[i.end] : null;
    return this;
  }
  lastChar() {
  ...
  slice(t = 0, e = this.original.length - this.offset) {
    ...
    for (; n && (n.start > t || n.end <= t); ) {
      if (n.start < e && n.end >= e) return i;
      n = n.next;
    }
    if (n && n.edited && n.start !== t) throw new Error(`Cannot use replaced character ${t} as slice start anchor.`);
    const r = n;
    for (; n; ) {
      n.intro && (r !== n || n.start === t) && (i += n.intro);
      const s = n.start < e && n.end >= e;
      if (s && n.edited && n.end !== e) throw new Error(`Cannot use replaced character ${e} as slice end anchor.`);
      const h = r === n ? t - n.start : 0, o = s ? n.content.length + e - n.end : n.content.length;
      if (i += n.content.slice(h, o), n.outro && (!s || n.end === e) && (i += n.outro), s) break;
      n = n.next;
    }
    return i;
  }
  ...
  _splitChunk(t, e) {
    if (t.edited && t.content.length) {
      ...
      throw new Error(`Cannot split a chunk that has already been edited (${n.line}:${n.column} \u2013 "${t.original}")`);
    }
    const i = t.split(e);
    return this.byEnd[e] = t, this.byStart[e] = i, this.byEnd[i.end] = i, t === this.lastChunk && (this.lastChunk = i), this.lastSearchedChunk = t, true;
  }
  toString() {
  ...
  isEmpty() {
    ...
    do
      if (t.intro.length && t.intro.trim() || t.content.length && t.content.trim() || t.outro.length && t.outro.trim()) return false;
    while (t = t.next);
    return true;
  }
  length() {
  ...
  trimEndAborted(t) {
    ...
    do {
      ...
      if (i.end !== n && (this.lastChunk === i && (this.lastChunk = i.next), this.byEnd[i.end] = i, this.byStart[i.next.start] = i.next, this.byEnd[i.next.end] = i.next), r) return true;
      i = i.previous;
    } while (i);
    return false;
  }
  ...
  trimStartAborted(t) {
    ...
    do {
      ...
      if (i.end !== n && (i === this.lastChunk && (this.lastChunk = i.next), this.byEnd[i.end] = i, this.byStart[i.next.start] = i.next, this.byEnd[i.next.end] = i.next), r) return true;
      i = i.next;
    } while (i);
    return false;
  }
  ...
  _replaceRegexp(t, e) {
    ...
    if (t.global) n(t, this.original).forEach((s) => {
      if (s.index != null) {
